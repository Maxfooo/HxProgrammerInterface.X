...
EEPROM_STATUS DATAEE_ReadByte(uint16_t bAdd, uint8_t *bData)
{
...
{  
    ...
    {
        DATAEE_WriteByte(eraseAddr++, EEPROM_ERASE_BYTE);
    }
...
EEPROM_STATUS EEPROM_Initialize()
{
    ...
    EEPROM_STATUS eepStat = DATAEE_ReadByte(org, &bData);
    ...
    eepStat = DATAEE_ReadByte(org+1, &bData);
    if (eepStat != EEPROM_READ_SUCCESS)
    {
        return eepStat;
    }
...
EEPROM_STATUS EEPROM_Update()
{
    ...
    EEPROM_STATUS eepStat = DATAEE_EraseBlock(org, org+1);
    if (eepStat != EEPROM_ERASE_SUCCESS)
    {
        return eepStat;
    }
    
    eepStat = DATAEE_WriteByte(org, (eeprom_currAddr & 0xFF));
    if (eepStat != EEPROM_WRITE_SUCCESS)
    {
        return eepStat;
    }
    
    eepStat = DATAEE_WriteByte(org+1, ((eeprom_currAddr >> 8) & 0xFF));
    if (eepStat != EEPROM_WRITE_SUCCESS)
    {
        return eepStat;
    }
